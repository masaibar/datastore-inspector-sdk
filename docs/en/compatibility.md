...
### Automatic Proto schema mapping

For non-minified debuggable variants, the Plugin automatically maps Proto2 and Proto3 Java Lite messages generated by the Google protobuf Gradle Plugin in the application or reachable first-party Android project modules. It resolves `java_package`, `java_outer_classname`, `java_multiple_files`, nested messages, and default outer-class collisions with message, enum, or service names from the descriptors. Kotlin protobuf DSL usage is also supported when the DataStore value remains the same generated Java Lite message.

The Stable `schemaEntry` API remains available as an explicit mapping when a descriptor is collected but automatic naming is outside the supported scope. If automatic and explicit mappings point the same generated class to different messages, the build fails instead of guessing another schema.
...
`schemaEntry` does not supply a descriptor, so it cannot enable a schema available only in an external binary without a collected descriptor.
