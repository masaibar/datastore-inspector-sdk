...
The Plugin automatically maps Proto schemas that meet these conditions:

- a non-minified debuggable variant
- a descriptor from the application or a reachable first-party Android project module
- a Proto2 or Proto3 Java Lite message generated by the Google protobuf Gradle Plugin
- `java_package`, `java_outer_classname`, `java_multiple_files`, nested messages, and default outer-class name collisions

Kotlin protobuf DSL is also supported when the DataStore value remains the same generated Java Lite message.

#### Explicit mapping

When a descriptor is collected but automatic naming is unsupported, use the Stable `schemaEntry` API to provide an explicit mapping.
...
- The build fails if automatic and explicit mappings point the same generated class to different messages.
- `schemaEntry` does not supply a descriptor and cannot enable schemas available only in external binaries.
